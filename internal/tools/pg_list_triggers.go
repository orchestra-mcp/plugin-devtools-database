package tools

import (
	"context"
	"fmt"
	"strings"

	pluginv1 "github.com/orchestra-mcp/gen-go/orchestra/plugin/v1"
	"github.com/orchestra-mcp/plugin-devtools-database/internal/db"
	"github.com/orchestra-mcp/sdk-go/helpers"
	"google.golang.org/protobuf/types/known/structpb"
)

func PgListTriggersSchema() *structpb.Struct {
	s, _ := structpb.NewStruct(map[string]any{
		"type": "object",
		"properties": map[string]any{
			"connection_id": map[string]any{"type": "string", "description": "Connection ID"},
			"table":         map[string]any{"type": "string", "description": "Filter by table name (optional)"},
		},
		"required": []any{"connection_id"},
	})
	return s
}

func PgListTriggers(mgr *db.Manager) func(ctx context.Context, req *pluginv1.ToolRequest) (*pluginv1.ToolResponse, error) {
	return func(ctx context.Context, req *pluginv1.ToolRequest) (*pluginv1.ToolResponse, error) {
		if err := helpers.ValidateRequired(req.Arguments, "connection_id"); err != nil {
			return helpers.ErrorResult("validation_error", err.Error()), nil
		}
		connID := helpers.GetString(req.Arguments, "connection_id")
		table := helpers.GetString(req.Arguments, "table")

		pg, errResp := getPostgresProvider(mgr, connID)
		if errResp != nil {
			return errResp, nil
		}

		triggers, err := pg.ListTriggers(ctx, table)
		if err != nil {
			return helpers.ErrorResult("list_triggers_error", err.Error()), nil
		}
		if len(triggers) == 0 {
			return helpers.TextResult("No triggers found."), nil
		}

		var b strings.Builder
		b.WriteString("| Trigger | Table | Timing | Events | Level | Function | Enabled |\n")
		b.WriteString("|---------|-------|--------|--------|-------|----------|--------|\n")
		for _, t := range triggers {
			fmt.Fprintf(&b, "| %s | %s | %s | %s | %s | %s | %s |\n",
				t.Name, t.Table, t.Timing, strings.Join(t.Events, ", "),
				t.Level, t.Function, t.Enabled)
		}
		return helpers.TextResult(b.String()), nil
	}
}
